...
export default function Phase3EmployeeScreen() {
  ...
  useEffect(() => {
    if (!batchId) {
      setMessage("A Phase 3 batchId is required.");
      return;
    }
    loadPhase3Work(batchId)
      .then(({ results }) => {
        setItems(results);
        const restored: Record<string, string | number | boolean> = {};
        for (const { result } of results)
          for (const question of result.questions) {
            const saved = question.answer?.value;
            if (["string", "number", "boolean"].includes(typeof saved))
              restored[`${result.itemId}:${question.id}`] = saved as
                string | number | boolean;
          }
        setValues(restored);
        setMessage(
          results.some(({ result }) => result.questions.length)
            ? ""
            : "No employee questions remain for this batch.",
        );
      })
      .catch((error) =>
        setMessage(
  ...
  async function submit(work: WorkItem) {
    ...
    if (Object.keys(answers).length === 0) {
      setMessage("Answer at least one generated question before saving.");
      return;
    }
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Unable to save answers.",
      );
    }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main
        ...
        data-testid="phase3-employee-work-screen"
      >
        <h1 className="text-2xl font-bold">Phase 3 Employee Work Screen</h1>
        <p className="mt-1 text-sm text-slate-600">
          Only deterministic questions generated from the imported SixBit row
          are shown.
        </p>
        {message && (
          <div role="status" className="my-4 rounded border bg-white p-3">
            {message}
          </div>
        )}
        <div className="mt-5 space-y-5">
          {items
            ...
            .map((work) => (
              <section
                key={work.result.itemId}
                className="rounded-lg border bg-white p-5 shadow-sm"
              >
                <h2 className="font-semibold">SKU: {work.item.sku}</h2>
                <p className="mb-4 text-sm text-slate-600">
                  {String(
                    work.result.normalizedValues.title ?? "Untitled listing",
                  )}
                </p>
                ...
                <Button className="mt-4" onClick={() => void submit(work)}>
                  Save answers and revalidate
                </Button>
              </section>
            ))}
        </div>
