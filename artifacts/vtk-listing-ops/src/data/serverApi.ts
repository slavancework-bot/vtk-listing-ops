...
export async function saveDraft(itemId:string,draft:ItemDraft,expectedDraftVersion:number){return request<{draftVersion:number}>(`/items/${itemId}/draft`,{method:"PUT",body:JSON.stringify({draft,expectedDraftVersion})});}
export async function saveFinal(itemId:string,draft:ItemDraft,review:boolean,idempotencyKey:string){const drop=window.localStorage.getItem("vtk-drop-next-response")===(review?"review":"answer");if(drop)window.localStorage.removeItem("vtk-drop-next-response");return request<{itemId:string;itemVersion:number;status:string;nextItemId:string|null;replayed:boolean}>(`/items/${itemId}/${review?"needs-review":"answer"}`,{method:review?"POST":"PUT",headers:{"idempotency-key":idempotencyKey,...(drop?{"x-test-drop-response-after-commit":"true"}:{})},body:JSON.stringify(review?{draft,reason:{code:"workflow_exception",note:"Employee requested review."}}:{draft})});}
