...
import { NonproductionFilesystemStorage, type FileStorage } from "./file-storage";
...
test("all seven Phase 1 tables and append-only audit protection exist", async () => {
  ...
  assert.deepEqual(tables.rows.map((row) => row.table_name).filter((name) => !name.startsWith("__drizzle")), ["audit_events", "batches", "idempotency_records", "imported_files", "item_drafts", "listing_analyses", "listing_exports", "listing_items", "listing_questions", "processing_jobs", "review_records"]);
...
test("real Phase 3 PostgreSQL path preserves originals through analysis, evidence, review, and export", async () => {
  ...
  const storage: FileStorage = { store: async (content) => { const key = `phase3-${stored.size + 1}.csv`; stored.set(key, content); return { key, remove: async () => { stored.delete(key); } }; } };
  const access = new Phase2Service(storage); const service = new Phase3Service(storage, access);
  const content = await readFile(new URL("../../../../fixtures/phase3-sixbit-v1.csv", import.meta.url), "utf8");
  const input = { actorId: "phase3-owner", role: "employee", importKey: "phase3-real-path", filename: "sixbit-v1.csv", mimeType: "text/csv", content, correlationId: "00000000-0000-4000-8000-000000000070" };
  const imported = await service.import(input); assert.equal(imported.itemCount, 4); assert.equal(imported.replayed, false); assert.equal(stored.size, 1); assert.equal((await service.import(input)).replayed, true); assert.equal(stored.size, 1);
  const before = await pool.query<{ id: string; original_values: Record<string,string>; stock: string }>("select id, original_values, original_values->>'StockTotal' as stock from listing_items where batch_id=$1 order by source_row_number", [imported.batchId]);
  const summary = await service.analyzeBatch(imported.batchId, "phase3-owner", "employee", "00000000-0000-4000-8000-000000000071"); assert.equal(summary.total, 4); assert.ok(summary.verify >= 2);
  await service.answer(before.rows[2].id, "phase3-owner", "employee", "r2-evidence", { "q:r2.required": "R2-F3-VERIFIED" }, "00000000-0000-4000-8000-000000000072");
  await service.answer(before.rows[3].id, "phase3-owner", "employee", "qty-evidence", { "q:qty.override": true }, "00000000-0000-4000-8000-000000000073");
  for (const row of before.rows) { const result = await service.getResult(row.id, "phase3-reviewer", "reviewer"); if (!result.exportReady) { const key=`review-${row.id}`; assert.equal((await service.review(row.id, "phase3-reviewer", "reviewer", key, { status: "approved", reason: "Synthetic acceptance fixture verified." }, "00000000-0000-4000-8000-000000000074")).replayed, false); assert.equal((await service.review(row.id, "phase3-reviewer", "reviewer", key, { status: "approved", reason: "Synthetic acceptance fixture verified." }, "00000000-0000-4000-8000-000000000075")).replayed, true); } }
  const exported = await service.exportBatch(imported.batchId, "phase3-reviewer", "reviewer"); assert.equal(exported.rowCount, 4); assert.match(exported.content, /UnknownPassThrough/); assert.match(exported.content, /keep-three/); assert.match(exported.content, /'=preserve-literal/);
  const after = await pool.query<{ original_values: Record<string,string>; stock: string }>("select original_values, original_values->>'StockTotal' as stock from listing_items where batch_id=$1 order by source_row_number", [imported.batchId]); assert.deepEqual(after.rows, before.rows.map(({ original_values, stock }) => ({ original_values, stock })), "original row JSON and StockTotal remain immutable");
  assert.equal((await pool.query("select 1 from listing_exports where batch_id=$1", [imported.batchId])).rowCount, 1); assert.equal((await pool.query("select 1 from listing_questions q join listing_items i on i.id=q.item_id where i.batch_id=$1", [imported.batchId])).rowCount! >= 2, true);
});
