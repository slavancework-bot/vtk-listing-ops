...
test("real filesystem import replay stores one file and failed import cleans its staged file",async()=>{const root=await mkdtemp(join(tmpdir(),"vtk-real-import-"));const prior=process.env.ALLOW_NONPRODUCTION_STORAGE;try{process.env.ALLOW_NONPRODUCTION_STORAGE="true";const service=new Phase2Service(new NonproductionFilesystemStorage(root));const content=await readFile(new URL("../../../../fixtures/phase2-controlled-eight-scenarios-v1.csv",import.meta.url),"utf8");const input={actorId:"real-storage-actor",role:"employee",importKey:"real-storage-replay",filename:"real.csv",mimeType:"text/csv",content,correlationId:"00000000-0000-4000-8000-000000000061"};const first=await service.importBatch(input);const replay=await service.importBatch(input);assert.equal(replay.batchId,first.batchId);assert.equal(replay.replayed,true);assert.equal((await readdir(root)).length,1);const duplicateRows=content.replace(",B,VTK-C1111-4P-14",",A,VTK-C1111-4P-14");await assert.rejects(()=>service.importBatch({...input,importKey:"real-storage-failure",content:duplicateRows,correlationId:"00000000-0000-4000-8000-000000000062"}));assert.equal((await readdir(root)).length,1,"failed import removes only its newly staged real file");}finally{if(prior===undefined)delete process.env.ALLOW_NONPRODUCTION_STORAGE;else process.env.ALLOW_NONPRODUCTION_STORAGE=prior;await rm(root,{recursive:true,force:true});}});
