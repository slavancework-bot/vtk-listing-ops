...
test("staging startup rejects a database not explicitly marked nonproduction",async()=>{const entry=fileURLToPath(new URL("../index.ts",import.meta.url));const result=await new Promise<{code:number|null;stderr:string}>((resolve)=>{const child=spawn(process.execPath,["--import","tsx",entry],{env:{...process.env,NODE_ENV:"test",APP_ENV:"staging",PORT:"39998",DATABASE_URL:"postgresql://user:pass@db.example/vtk_production",ALLOW_NONPRODUCTION_DATABASE:"true",ALLOW_DEVELOPMENT_IDENTITY:"true",ALLOW_NONPRODUCTION_STORAGE:"true",STAGING_FILE_ROOT:"test-files"},stdio:["ignore","ignore","pipe"]});let stderr="";child.stderr.on("data",(chunk)=>{stderr+=String(chunk);});child.on("close",(code)=>resolve({code,stderr}));});assert.notEqual(result.code,0);assert.match(result.stderr,/test, staging, or nonproduction/i);});
