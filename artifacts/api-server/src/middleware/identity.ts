...
export class DevelopmentIdentityProvider implements IdentityProvider {
  async authenticate(req: Request) {
    ...
    const requestedRole = req.header("x-development-role") ?? "employee";
    if (
      !(["employee", "reviewer", "admin"] as string[]).includes(requestedRole)
    )
      return null;
    return {
      subject,
      role: requestedRole as "employee" | "reviewer" | "admin",
    };
  }
}
...
export function requireIdentity(provider: IdentityProvider) {
  return async (req: Request, _res: Response, next: NextFunction) => {
    try {
      ...
      if (!identity)
        throw new ApiFault(401, "UNAUTHORIZED", "Authentication is required.");
