...
export function createPhase2Router(service: Phase2Service) {
  ...
  router.get("/batches/:batchId/progress", requireRole("employee","reviewer","admin"), async (req,res,next)=>{try{res.json(await service.getProgress(id(GetBatchProgressParams,req.params),req.identity!.subject,req.identity!.role));}catch(error){next(error);}});
